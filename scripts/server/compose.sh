...
if [ "$CONFIG_TYPE" = "proxied" ]; then
  tmpfile=$(mktemp)
  envsubst '$SERVER_NAME' < $pre_cert_conf_path > $tmpfile && mv $tmpfile $pre_cert_conf_path
  tmpfile=$(mktemp)
  envsubst '$SERVER_NAME' < $post_cert_conf_path > $tmpfile && mv $tmpfile $post_cert_conf_path
